...
text4 = GameFrame(["... To go left, use A to move ...","... To go right, use D to move ...","... To stay put, press S ..."],[lambda: checkTime(settings.Time.NIGHT), lambda: toNextState(InStates.STAYPUT2)],[lambda: keyPressed("enter"),lambda: text4.disableText()],[lambda: keyPressed("s"),lambda: timeState(InStates.STAYPUT, settings.Time.SUNSET)],[lambda: checkPlayerRight(),lambda: timeState(InStates.RIGHT, settings.Time.SUNSET)],[lambda: checkPlayerLeft(),lambda: timeState(InStates.LEFT, settings.Time.SUNSET)])
stayPut = GameFrame(["... You decide to stay put ...",f"... It is now {settings.times[settings.time]} ...","... You can either go left, right, or stay put ..."],[lambda: checkTime(settings.Time.NIGHT), lambda: toNextState(InStates.STAYPUT2)],[lambda: keyPressed("enter"),lambda: toNextState(InStates.TEXT4)])
stayPut2 = GameFrame(["... You decide to stay put ...",f"... It is now {settings.times[settings.time]} ...","... You pass out from exhaustion ..."],[lambda: keyPressed("enter"),lambda: toNextState(InStates.ENDSLEEPNIGHT)])
left = GameFrame(["... You decided to go left. ...",f"... You travel ten miles over the course of 5 hours. It is now {settings.times[settings.time]}. ...","... Where you stop seems to be exactly where you started. ..."],[lambda: keyPressed("enter"),lambda: toNextState(InStates.LEFTTXT2)])
...
leftTxt3 = GameFrame(["... To sleep on the road, press S ...","... To go into the forest, press W ...","... To keep walking, use A and D"],[lambda: keyPressed("enter"),lambda: leftTxt3.disableText()],[lambda: keyPressed("s"),lambda: toNextState(InStates.ENDSLEEPNIGHT)],[lambda: keyPressed("w"),lambda: toForest()],[lambda: checkPlayerLeft(),lambda: timeState(InStates.ENDWALK, settings.Time.NIGHT)],[lambda: checkPlayerRight(),lambda: timeState(InStates.ENDWALK, settings.Time.NIGHT)])
right = GameFrame(["... You decided to go right. ...",f"... You travel ten miles over the course of 5 hours. It is now {settings.times[settings.time]}. ...","... Where you stop seems to be exactly where you started. ..."],[lambda: keyPressed("enter"),lambda: toNextState(InStates.LEFTTXT2)])
...
returnToFire = GameFrame([" ... ...","... You return to the campfire ...","... ..."],[lambda: keyPressed("enter"),lambda: timeState(InStates.CAMP2, settings.Time.NIGHT)])
camp2 = GameFrame(["... You can either sleep in the tent (S) ...","... Rekindle the fire (F) ...","... Or look for more sticks (W) ..."],[lambda:  keyPressed("s"),lambda: toNextState(InStates.ENDSLEEPNIGHT)],[lambda: keyPressed("f"),lambda: toNextState(InStates.REKINDLEFIRE)],[lambda: keyPressed("w"),lambda: toNextState(InStates.ENDATTACKNIGHT)])
endAttackedNight = GameFrame(["... You go out in the night ...","... but are attacked by a wild animal ...","... Unfortunately, you are killed. GAME OVER. ..."],[lambda: keyPressed("enter"),lambda: toGameState(States.ENDGAMELOSE)])
fRekindleFire = GameFrame(["... You attempt to rekindle the fire ...","... ...","... but you fail ..."], [lambda: keyPressed("enter"),lambda: toNextState(InStates.FORESTTXT2)])
rekindleFire = GameFrame(["... You attempt to rekindle the fire ...","... ...","... and you succeed! ..."], [lambda: keyPressed("enter"),lambda: setPlayerDamageAdder(12, InStates.CHOICE4)])
